...
<tr><td style="background-color: #FFF">Nama Kategori</td><td>:</td><td style="background-color: #FFF"> <input name="namaKat" type="text" size="54"	 required="required" value="<?php $kategori=mysql_fetch_array(mysql_query("SELECT * FROM kategori where id_kategori='$_GET[id_kat]'")); 
echo($kategori['content']);?>" /></td></tr>
<tr><td></td><td></td><td>
<input type="hidden" name="e" value="editKat" />
<input type="hidden" name="id" value=<?php echo $_GET['id_kat'];?> />
<input type="submit" value="Edit" align="middle" />
</form></td></tr>
</table>  
        </div>
