...
<div class="container">
    <div id="kotak_nav_atas">
          <div id="stylelink_atas"> <a href="default.php">Beranda </a> <a href="pesan.php">Pesan </a> <?php echo "<a href=\"profil.php?user=".$_SESSION['u']."\">".$_SESSION['jeneng']."</a>"?> 
          <?php if($_SESSION['u']=="admin"){
			   echo "<a href=\"lihat-Member.php\">Lihat Member</a>";
			   echo("<a href=\"badwords.php\">Lihat Bad Words</a>");
		   }?>
           <a href="emoticon.php">List Emoticon</a><a href="logout.php">Logout </a> </div></div>
          <h3>Lihat Member</h3>

<br /><br />
<?php
if($_SESSION['u']!="admin"){
echo "Anda tidak mempunyai akses untu melihat halaman ini";
}
else{

$allmember=mysql_query("SELECT * FROM user");
echo "<table border=\"2px\" bgcolor=\"#00FFCC\">";
while($semuamember=mysql_fetch_array($allmember)){
	$xxx=$semuamember['avatar'];
	echo("<tr><td><a href=\"profil.php?user=$semuamember[username]\"><img src='$xxx' width=\"50\" height=\"50\"></a>");
	$xxx="hapusMember";
	echo "</td><td>$semuamember[nama]<br></td><td><a href=\"proses-Hapus.php?e=$xxx&user=$semuamember[username]\"><linkTambahKat>Delete</linkTambahKat></td><tr>";
}
}
?>
