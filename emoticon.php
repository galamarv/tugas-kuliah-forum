...
<div id="wrapper";>
<div class="header"><a href="http://www.ub.ac.id" target="_blank"><img src="image/LI.png" height="200px" width="230px"/></a></div>
...
<input type="hidden" name="e" value="tambahEmot"/>
<table>
<tr><td width="200">Emot</td><td><input type="text" name="emot" required="required"/></td></tr>
<tr><td>Gambar</td><td><input type="file" name="gambar" required="required"/></td></tr>
<tr><td></td><td><input type="submit" value="Tambah" /></td></tr>
           </table>
</form>
<?php }?>
<?php
$allemot=mysql_query("SELECT * FROM emot");
echo "<h3>Daftar Emoticon</h3>";
?><table border="2px" bgcolor="#00FFCC"><?php 
while($dataemot=mysql_fetch_array($allemot)){?>
<tr><td bgcolor="#00CCCC"><?php echo "$dataemot[id_emot]"?></td>
<td><img src="<?php echo "$dataemot[emot]"?>"></td>
 <?php if($_SESSION['u']=="admin"){
	 $e="hapusEmot";
	 $id_emot=("$dataemot[id_emot]");
echo "<td><a href=\"proses-Hapus.php?e=$e&id=$id_emot\" style=\"color:rgb(0,153,255)\">Delete</a></td>";
		}
		echo "</tr>";
		   }
